function perceptron(){
	...
	this.studyNew = function(sourc){
	
		var train = sourc.concat();
	...
	function CREATE(){
		...
		for(var i = 1; i < layers; i++){
			...
			for(var j = 0; j < neuron[i - 1]; j++){
				neural.layers[i].neuron.push({
					w: [],
					v: 0,
					e: 0
				});
	
				for(var k = 0; k < neural.layers[i - 1].neuron.length; k++ ){
					neural.layers[i].neuron[j].w.push(Math.random() - 0.5);
				}
			}
		}
