function cnvs_ui(options){
	...
	var CREATE = {
		...
		colors: function(){
			...
			for(var i = 0; i < 100; i++){
				...
				var mainColor = [0, 0, 0];
				mainColor[Math.round(Math.random()*2)] = 1;

				var f = (100 + Math.round(Math.random()*100 + mainColor[0]*50 )).toString(16);
				var s = (100 + Math.round(Math.random()*100 + mainColor[1]*50 )).toString(16);
				var t = (100 + Math.round(Math.random()*100 + mainColor[2]*50 )).toString(16);
	...
	var DISPLAY = new function(){
		...
		this.canvas = function(){
 		...
 		var gides = {
 			v: function(c){
 				fline = (Math.floor((s.l + 2)/5) + 1)*5;
			
 				c.lineWidth = 0.1*_size.scale;
				c.strokeStyle = '#777777';
				c.fillStyle = '#777777';
	
				c.textAlign = "center";
				c.textBaseline = "bottom";
				c.font = 1*_size.scale + 'px sans-serif';
	
				c.beginPath();
				while(fline < s.l + s.width){
					c.fillText(
						fline,
						_size.scale * (fline + 0.1),
						_size.scale * (s.height + s.t - 0.1));
					c.moveTo(
						_size.scale * (fline),
						_size.scale * (s.t) );	
					c.lineTo(
						_size.scale * (fline),
						_size.scale * (s.height + s.t - 2) );
					fline += 5;
				}
				c.stroke();
				c.closePath();
 		...
 		var draw = {
 			g: function(c){
				...
				label.string(c, [ -8, 112 ], '0');
				label.string(c, [ 0, -8 ], '100%');

				for(var i = 1; i < 20; i++){

					label.string(c, [ 12 + 100*i, 112 ], 100*i);
				...
				c.beginPath();
				c.lineWidth = 0.6*_size.scale;

				c.moveTo(
					_size.scale * 0,
					_size.scale * ( 100 - _sources.g[0]*100)
				);

				for(var i = 1; i < _sources.g.length; i++){
					c.lineTo(
						_size.scale * i,
						_size.scale * ( 100 - _sources.g[i]*100)
					);
				}
				c.stroke();
				c.closePath();
 			...
 			d: function(c){
				...
				for(var j = 0; j < _sources.d.length; j++){
					c.beginPath();
					c.strokeStyle = colors[j];
	
					c.moveTo(
						_size.scale * 0,
						_size.scale * ( 100 - _sources.d[j][0]*100)
					);
	
	
					for(var i = 1; i < _sources.d[j].length; i++){
						c.lineTo(
							_size.scale * i,
							_size.scale * ( 100 - _sources.d[j][i]*100)
						);
					}
					c.stroke();
					c.closePath();
